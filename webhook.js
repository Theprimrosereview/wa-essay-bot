...
async function getOrCreateSession(msisdn) {
  const { data, error } = await supabase.from("wa_sessions").select("*").eq("msisdn", msisdn).limit(1).maybeSingle();
  if (error) throw error;
  if (data) return data;
  const { data: created, error: insertErr } = await supabase.from("wa_sessions").insert({ msisdn, step: "welcome" }).select("*").single();
